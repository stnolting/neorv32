...
:information_source: If the simulation of a test does not generate any signature outputs at all or if the signature is truncated
try increasing the simulation time in the NEORV32 port's device makefiles (modify `--stop_time`, example: `sh $(NEORV32_LOCAL_COPY)/sim/ghdl/ghdl_sim.sh --stop-time=900us`).
