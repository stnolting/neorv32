...
The size of those FIFOs can be configured by the `IO_TWD_RX_FIFO` and `IO_TWD_TX_FIFO` generics. Software can determine the FIFO size via the
`TWD_CTRL_FIFO_*` bits.
...
pulse. The TWD will read a byte from the internal TX FIFO or dummy byte and will transmit it MSB-first to the host (blue signals `D7`
to `D0)`. When no data is available and the dummy byte is disabled, the TWD will not acknowledge the transaction and waits for the next one.
During the 9th clock pulse the host can acknowledged the transfer (**ACK**) to get the next byte immediately. If no ACK (**NACK**) is received by the
TWD, the host does not await the new byte. If the TX FIFO becomes
empty while the host keeps reading data, the dummy is transmitted, when enabled, else `0xFF` is ts transmitted. The transaction is terminated when the host
...
.20+<| `0xffea0000` .20+<| `CTRL` <|`0`     `TWD_CTRL_EN`                                   ^| r/w <| TWD enable, reset if cleared
                                  ...
                                  <|`14`    `TWD_CTRL_DUMMY_EN`                            ^| r/w <| Enable TX dummy byte
...
.2+<| `0xffea0008` .2+<| `DUMMY` <|`7:0`   `TWD_DUMMY_MSB : TWD_DUMMY_LSB`               ^| r/w <| RX dummy byte register access
                                  <|`31:8`  -                                               ^| r/- <| _reserved_, read as zero
|=======================
