...
read a byte from the internal TX FIFO and will transmit it MSB-first to the host. During the 9th clock pulse the host
acknowledges the transfer (ACK) by pulling SDA low, or signals the end of the transfer with a NACK (SDA high). The byte
is taken from the TX FIFO in both cases, as it has been delivered to the host. After a NACK the TWD releases the bus and
waits for a STOP or (repeated) START condition, as required by the I²C-bus specification. If the TX FIFO becomes empty
while the host keeps reading data, all-one bytes are sent back to the host. The transaction is terminated by a STOP condition.
...
the ACK/NACK decision is taken once at the beginning of the 9th clock and kept for the whole clock pulse, and a NACKed byte
is not stored. The transaction is terminated by a STOP condition.
