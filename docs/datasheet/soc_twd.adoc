...
.TWD read operation timing (split across two lines)
...
.TWD write operation timing (split across two lines)
[wavedrom, format="svg", align="center"]
----
{signal: [
...
Actual communication with the TWD is **started** when a START (or REPEATED-START) condition followed by the TWD device
...
* `TWD_CTRL_IRQ_RX_AVAIL`: trigger interrupt if at least one data byte is available in the RX FIFO (i.e. `TWD_CTRL_RX_AVAIL` is set)
* `TWD_CTRL_IRQ_RX_FULL`: trigger interrupt if the RX FIFO is completely full (i.e. `TWD_CTRL_RX_FULL` is set)
* `TWD_CTRL_IRQ_TX_EMPTY`: trigger interrupt if the TX FIFO is empty (i.e. `TWD_CTRL_TX_EMPTY` is set)
* `TWD_CTRL_IRQ_COM_BEG`: trigger interrupt if a communication-start has been observed (i.e. `TWD_CTRL_COM_BEG` is set)
* `TWD_CTRL_IRQ_COM_END`: trigger interrupt if a communication-end has been observed (i.e. `TWD_CTRL_COM_END` is set)
...
.20+<| `0xffea0000` .20+<| `CTRL` <|`0`     `TWD_CTRL_EN`                                   ^| r/w <| TWD enable, reset if cleared
                                  ...
                                  <|`14`    `TWD_CTRL_IRQ_COM_BEG`                          ^| r/w <| IRQ if begin of communication
                                  <|`15`    `TWD_CTRL_IRQ_COM_END`                          ^| r/w <| IRQ if end of communication
