...
* Register RX mode where a host read operation always reads this byte
...
When the Register RX mode ist enabled the RX Fifo gets cleared a host read operation always reads the `TX_REG` byte.
...
.20+<| `0xffea0000` .20+<| `CTRL` <|`0`     `TWD_CTRL_EN`                                   ^| r/w <| TWD enable, reset if cleared
                                  ...
                                  <|`14`    `TWD_CTRL_TX_REG_EN`                            ^| r/w <| Enable TX reg mode (instead of FIFO)
...
.2+<| `0xffea0008` .2+<| `TX_REG` <|`7:0`   `TWD_TX_REG_MSB : TWD_TX_REG_LSB`               ^| r/w <| RX data register access
                                  <|`31:8`  -                                               ^| r/- <| _reserved_, read as zero
|=======================
