...
to `D0)`.
During the 9th clock pulse the host can acknowledged the transfer (**ACK**) to get the next byte immediately.
If no ACK (**NACK**) is received by the TWD, the host does not await the new byte.
If the TX FIFO is empty, the dummy is transmitted, when enabled, else `0xFF` is ts transmitted.
The transaction is terminated when the host issues a **STOP** condition.
