...
The bootloader can access an TWI-compatible memory via the processor's top entity TWI port. Only single-address memory is supported, and reading is done in the following pattern
`Slave Address + Enabled Read | Memory Address | Read Byte 0 | Read Byte 1 | Read Byte 2 | Read Byte 3`.
