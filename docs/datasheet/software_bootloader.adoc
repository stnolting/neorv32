...
<<_auto_boot_sequence>> is started. This auto-boot sequence can be skipped within 8s by pressing any key.
...
ultimately defined by the <<_linker_script>> using the `__neorv32_rom_base` symbol. This also corresponds to the entry
