...
Duty cycle and period length of each channel are defined by the according `CHANNEL` registers. These registera are
...
load/store operation. This allows to update `TOP` and `CMP` independant of each other.
