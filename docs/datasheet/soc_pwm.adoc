...
* Up to 16 individual channels with 16-bit resolution
...
The PWM module implements a fast-PWM mode pulse-width modulation controller with up to 16 independent channels.
...
The total number of implemented channels is defined by the `IO_PWM_NUM_CH` generic. The PWM output signal `pwm_o`
has a static size of 16-bit. Channel 0 corresponds to bit 0, channel 1 to bit 1 and so on. If less than 16 channels
...
register and up to 16 `CHANNEL[i]` registers.
...
lowest `IO_PWM_NUM_CH` bit are implemented.

The `POLARITY` register defines the _idle polarity_ of the PWM channels. When a channel is disabled, this polarity
also defines the logic level of the according `pwm_o` output. Note that only the lowest `IO_PWM_NUM_CH` bit are
implemented.
...
`0 .. IO_PWM_NUM_CH`). This register is split in two half-words: the lowest half-word defines the counter-compare
value (`CMP`) while the upper half-word defines the counter-wrap value (`TOP`).

.`CHANNEL` Half-Word Access
[TIP]
In contrast to many other ENORV32 peripherals, the `CHANNEL[i]` registers can also be accessed using half-word
...
| `0xfff00000` | `ENABLE`   | `15:0` | r/w | Channel enable, one bit per channel
| `0xfff00004` | `POLARITY` | `15:0` | r/w | Channel idle polarity level, one bit per channel
| `0xfff00008` | `CLKPRSC`  | `2:0`  | r/w | Clock prescaler select
.2+<| `0xfff00040` .2+<| `CHANNEL[0]`  <|`31:16` - `TOP` ^| r/w <| Channel 0 top/wrap value register
                                       <|`15:0`  - `CMP` ^| r/w <| Channel 0 compare value register
.2+<| `0xfff00044` .2+<| `CHANNEL[1]`  <|`31:16` - `TOP` ^| r/w <| Channel 1 top/wrap value register
                                       <|`15:0`  - `CMP` ^| r/w <| Channel 1 compare value register
| ... | ...  | ... | r/w | Channels 2 to 14
.2+<| `0xfff0007c` .2+<| `CHANNEL[15]` <|`31:16` - `TOP` ^| r/w <| Channel 15 top/wrap value register
                                       <|`15:0`  - `CMP` ^| r/w <| Channel 15 compare value register
|=======================
