...
. Install an endless loop as trap handler to <<_mtvec>> CSR: the core will **halt** if any trap occurs ("crt0 panic").
. Clear <<_mie>> CSR disabling all interrupt sources.
. Initialize all integer register `x1` to `x31` (only `x1` to `x15` if `E` ISA extension is enabled).
...
. If `main` returns (which is not expected):
** Interrupts are disabled by clearing <<_mstatus>>.`MIE`.
** Reinstall the endless loop as trap handler ("crt0 panic").
** The return value of `main()` is copied to <<_mscratch>> CSR to allow inspection by the debugger.
** Call all _destructors_ (if there are any).
** CPU 0 will issue an `ebreak` instruction to try to transfer control to an external debugger; any other CPU will immediately halt (see next point).
** **Halt**: CPU 0 enters sleep mode executing `wfi` instruction in an endless loop ("crt0 panic").
