...
.RISC-V GCC C-API & ASM Reference Manuals
[TIP]
Copies of the official **RISC-V C API Specification** (https://github.com/riscv-non-isa/riscv-c-api-doc)
and the **RISC-V Assembly Programmer Manual** (https://github.com/riscv-non-isa/riscv-asm-manual) can be
found in the project's `docs` folder.
...
The compiled, assembled and linked ELF file (generated by the <<_application_makefile>> is further processed by
the NEORV32 executable image generator (`sw/image_gen`). This utility can generate various types of standalone
