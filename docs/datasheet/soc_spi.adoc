...
The SPI module supports all standard SPI clock modes (0, 1, 2, 3), which is via the two control register bits
...
.18+<| `0xffffffa8` .18+<| `NEORV32_SPI.CTRL` <|`0` _SPI_CTRL_CS0_    ^| r/w .8+<| Direct chip-select 0..7; setting `spi_csn_o(x)` low when set
                                              ...
                                              <|`16` _SPI_CTRL_CPOL_  ^| r/w <| clock polarity
                                              <|`17` .. `18`          ^| r/- <| _reserved, read as zero
