...
The entire address space layout can be customized via the "Processor Address Space Layout" constants section in the
processor's main VHDL package file (`rtl/core/neorv323_package.vhd`).
...
bus. Optionally, this bus is further switched by another instance of the bus switch so the <<_direct_memory_access_controller_dma>>
controller can also access the entire address space. Accesses via the resulting SoC bus are split by a **gateway**,
that redirects accesses to the according main memory regions. Accesses to the processor-internal IO/peripheral devices
are further redirected via a dedicated **IO switch**.
...
The gateway also implements a **bus monitor** that keeps track of all active bus requests. If an accessed address
responds with an error conditions or if no response is received at all an according bus access exception is raised.

**Maximum Response Latency**

The device at the accessed address does not have to respond within a fixed cycle amount after a bus request has been initiated.
However, the bus transaction has to be completed (= acknowledged) within a certain **response time window**. This time window
is defined by the global `max_proc_int_response_time_c` constant (default = 15 cycles) in the processor's VHDL package file.
...
accessing "address space holes" - the GATEWAY will issue a bus error to the CPU that will raise a bus access error
exception according to the timed-out bus access type (instruction fetch / load access /store access).

.XIP Timeout
[NOTE]
Accesses to the memory-mapped XIP flash will _never_ time out.
