...
address space from each of the interface. Both of them can be equipped with optional caches (<<_data_cache_dcache>>
and <<_instruction_cache_icache>>).
...
. The <<_data_cache_dcache>> and <<_instruction_cache_icache>>
...
clear the CPU's <<_instruction_cache_icache, instruction cache>>.
* `fence` (<<_i_isa_extension>> / <<_e_isa_extension>>): clear and reload the CPU's
<<_data_cache_dcache, data cache>>. Flushing is not required as the data cache
...
<<_bootloader_rom_bootrom>> is enabled. This ROM contains the executable image (`rtl/core/neorv32_bootrom_image.vhd`)
...
The ROM is initialized during synthesis with the according application image file (`rtl/core/neorv32_imem_image.vhd`).
