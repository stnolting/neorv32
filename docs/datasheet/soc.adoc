...
[TIP]
The interrupt request signals have specific `mip` CSR bits (see <<_machine_trap_setup>>), specific
`mie` CSR bits (see <<_machine_trap_handling>>) and specific `mcause` CSR trap codes and trap
...
| 10      | _SLINK_ | RX data received
| 11      | _SLINK_ | TX data send
| 12:15   | - | _reserved_
|=======================
...
entity signal that can be used to signal critical system conditions. This interrupt source _cannot_ be disabled.
