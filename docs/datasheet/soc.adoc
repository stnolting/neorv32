...
from the <<_system_configuration_information_memory_sysinfo>> module to make clock-specific configurations (like timer values,
interface timing).
|======
...
to each of these regions. These "access permission" are enforced by the hardware and cannot be changed. However, the access permissions
can be further constrained using the CPU's <<_pmp_physical_memory_protection>>.
