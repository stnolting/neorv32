...
* _optional_ quadrature decoder for rotary encoder with up to 6 channel (<<_quadrature_decoder_qdec,**QDEC**>>)
...
4+^| **Quadrature Decoder (<<_quadrature_decoder_qdec,QDEC>>)**
| `qdec_a_i` | 6 | in | rotary encoder phase A for channel 0..5
| `qdec_b_i` | 6 | in | rotary encoder phase B for channel 0..5
...
The value has to be a power of two and has to be greater than or equal to _two_ (>= 2).
...
[frame="all",grid="none"]
|======
| **IO_QDEC_NUM_CH** | _natural_ | 0
3+| Number of rotary encoder input channels (0..6) to implement. The QDEC module is _not_ implemented if zero.
See section <<_quadrature_decoder_qdec>> for more information.
|======
...
| 13      | <<_quadrature_decoder_qdec,QDEC>> | Quadrature decoder interrupt
