...
* **Arithmetic:** BCD (binary-coded decimal) operations; multiply-add operations
...
allows to implement up to 4 custom control and status registers (see <<_cfu_control_and_status_registers_cfu_csrs>>).
...
Non-Standard Encoding Space"). The NEORV32 CFU uses the `custom-1` and `custom-2` opcodes to identify the instruction
