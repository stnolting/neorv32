...
| `6`  | _SYSINFO_CPU_ZXSCNT_    | Custom extension - _Small_ CPU counters: `[m]cycle` & `[m]instret` CSRs have less than 64-bit when set
| `7`  | _SYSINFO_CPU_ZXNOCNT_   | Custom extension - _NO_ CPU counters: `[m]cycle` & `[m]instret` CSRs are NOT available at all when set
...
| `30  | _SYSINFO_CPU_FASTMUL_   | fast multiplication available when set
| `31` | _SYSINFO_CPU_FASTSHIFT_ | fast shifts available when set
|=======================
