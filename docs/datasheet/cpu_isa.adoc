...
fields are zero, the instruction is executed as a NOP without triggering a cache flush.
...
its instruction fetches. When executed, the CPU flushes the data cache (if enabled), clears the CPU's instruction
prefetch buffer and invalidates the instruction cache (if enabled). The next instruction fetch request
