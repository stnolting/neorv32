...
The execution trace port provides information to trace the CPU's instruction execution. Trace data is generated by the CPU's
trace generator (`neorv32_cpu_trace.vhd`) and sent to the <<_execution_trace_buffer_tracer>> module. Note that this port
can be enabled/disabled by the according <<_cpu_tuning_options>>.
...
| `valid` |     1 | All others signals are valid when set; high for one cycle
| `order` |    32 | Instruction index (linear incrementing counter; resets to zero on overflow)
| `pc`    |    32 | Instruction address (program counter)
| `inst`  |    32 | Instruction word; compressed instructions are shown in their decompressed 32-bit format
| `intr`  |     1 | Tap entry; set when this is the first instruction of a trap handler
| `mode`  |     1 | Operation / privilege mode; `0` = user-mode, `1` = machine-mode
| `debug` |     1 | Set when the instruction is executed in debug-mode
| `compr` |     1 | Current instruction is a de-compressed instruction
|=======================
...
| Description | When this option is **enabled**, the CPU code generates a full execution trace that can be consumed by the
