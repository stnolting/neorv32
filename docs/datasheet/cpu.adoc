...
as soon as the front-end (instruction fetch) and back-end (instruction execution) have halted. In this state there
are no pending bus accesses so other bus controllers like the DMA have full access to the bus system.
...
continue to buffer/enqueue incoming interrupt.

The CPU will leave sleep mode as soon as any _enabled (via <<_mie>>) interrupt source becomes _pending_ or if a debug
session is started.
