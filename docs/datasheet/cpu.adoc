...
.AMO PMP Rule Violation
[NOTE]
The RISC-V specification requires that read-modify-write AMO operations raise a _store access exception_ when
they violate a PMP rule. However, NEORV32 will always raise a **load access violation** in this case.
...
This output port provides all the required information to make a full trace of the CPU's instruction execution.
It is connected to the <<_execution_trace_buffer_tracer>> module.
...
3+^| **In-Band: Signals are valid if `valid` is set**
| `valid` |     1 | All in-band signals are valid when set; high for one cycle
| `pc`    |    32 | Instruction address (PC)
| `inst`  |    32 | Instruction word; compressed instructions are shown in their decompressed 32-bit format
| `rvc`   |     1 | Current instruction is a de-compressed instruction
| `mode`  |     2 | Operation / privilege mode; `00` = user-mode, `01` = machine-mode, `1-` = debug-mode
3+^| **Out-of-Band: Signals are valid without `valid` being set**
| `delta` |     1 | Branch operation (i.e. non-linear PC change); high for one cycle
| `trap`  |     1 | Trap entry; high until trap handler is started
|=======================
...
{signal: [
  ...
  [
    ...
    {name: 'err',  wave: 'x......|x.0x0..x.'},
    {name: 'data', wave: 'x......|x.3x456x.', data: ['[0]', '[4]', '[8]', '[12]']},
  ]
],
...
The NEORV32 CPU features a mechanism to detect **unintended nested traps**.
This mechanism is loosely based on the RISC-V `Smdbltrp` specification.

Whenever any trap (synchronous or asynchronous) is taken into M-mode an internal status flag is set. This flag gets set
...
Some traps are marked as **fatal**, as they are not resumable in most cases. However, the corresponding trap handler
can be used to output debug information, to bring the system into a safe state or to perform a defined system reset.

.NEORV32 Trap Listing
[cols="1,4,8,10,2,2,2,2"]
[options="header",grid="rows"]
|=======================
...
| 10    | `0x00000007` | `TRAP_CODE_S_ACCESS`     | store access fault                   | PC     | ADR     | INS      |
| 11    | `0x00000005` | `TRAP_CODE_L_ACCESS`     | load access fault                    | PC     | ADR     | INS      |
8+^| **Interrupts** (_asynchronous_ to instruction execution)
| 12    | `0x80000010` | `TRAP_CODE_FIRQ_0`       | fast interrupt request channel 0     | I-PC   | 0       | 0        |
| 13    | `0x80000011` | `TRAP_CODE_FIRQ_1`       | fast interrupt request channel 1     | I-PC   | 0       | 0        |
| 14    | `0x80000012` | `TRAP_CODE_FIRQ_2`       | fast interrupt request channel 2     | I-PC   | 0       | 0        |
| 15    | `0x80000013` | `TRAP_CODE_FIRQ_3`       | fast interrupt request channel 3     | I-PC   | 0       | 0        |
| 16    | `0x80000014` | `TRAP_CODE_FIRQ_4`       | fast interrupt request channel 4     | I-PC   | 0       | 0        |
| 17    | `0x80000015` | `TRAP_CODE_FIRQ_5`       | fast interrupt request channel 5     | I-PC   | 0       | 0        |
| 18    | `0x80000016` | `TRAP_CODE_FIRQ_6`       | fast interrupt request channel 6     | I-PC   | 0       | 0        |
| 19    | `0x80000017` | `TRAP_CODE_FIRQ_7`       | fast interrupt request channel 7     | I-PC   | 0       | 0        |
| 10    | `0x80000018` | `TRAP_CODE_FIRQ_8`       | fast interrupt request channel 8     | I-PC   | 0       | 0        |
| 21    | `0x80000019` | `TRAP_CODE_FIRQ_9`       | fast interrupt request channel 9     | I-PC   | 0       | 0        |
| 22    | `0x8000001a` | `TRAP_CODE_FIRQ_10`      | fast interrupt request channel 10    | I-PC   | 0       | 0        |
| 23    | `0x8000001b` | `TRAP_CODE_FIRQ_11`      | fast interrupt request channel 11    | I-PC   | 0       | 0        |
| 24    | `0x8000001c` | `TRAP_CODE_FIRQ_12`      | fast interrupt request channel 12    | I-PC   | 0       | 0        |
| 25    | `0x8000001d` | `TRAP_CODE_FIRQ_13`      | fast interrupt request channel 13    | I-PC   | 0       | 0        |
| 26    | `0x8000001e` | `TRAP_CODE_FIRQ_14`      | fast interrupt request channel 14    | I-PC   | 0       | 0        |
| 27    | `0x8000001f` | `TRAP_CODE_FIRQ_15`      | fast interrupt request channel 15    | I-PC   | 0       | 0        |
| 28    | `0x8000000b` | `TRAP_CODE_MEI`          | machine external interrupt (MEI)     | I-PC   | 0       | 0        |
| 29    | `0x80000003` | `TRAP_CODE_MSI`          | machine software interrupt (MSI)     | I-PC   | 0       | 0        |
| 30    | `0x80000007` | `TRAP_CODE_MTI`          | machine timer interrupt (MTI)        | I-PC   | 0       | 0        |
|=======================
