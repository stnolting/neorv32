...
| <<_sdtrig_isa_extension,`Sdtrig`>>     | Debug trigger module extension (hardware breakpoint)          | <<_processor_top_entity_generics, `OCD_HW_BREAKPOINT`>>
|=======================
...
even if <<_mstatush>>.mie is cleared.
...
The NEORV32 CPU features a mechanism to detect unintended nested traps which is derived from the RISC-V `Smdbltrp`
specification. When a trap is to be taken into M-mode, if the `MDT` bit (machine-mode disable trap) in <<_mstatush>>
is currently 0, it is then set to 1, and the trap is delivered as expected. However, if `MDT` is already set to 1,
then this is an unexpected _double trap_ exception (a nested trap). The RISC-V-compliant "double-trap" exception code
16 (see <<_neorv32_trap_listing>>) is used to signal this event. `MDT` can be cleared manually to allow nesting of
traps. However, _unexpected_ nested / double traps lead to a loss of the trap context (`mcause`, `mepc`, `mtval`,
stack frame, etc.) and can therefore not be resumed.
