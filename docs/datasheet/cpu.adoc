...
| Description | When **enabled** all conditional branch instruction will have identical execution times for taken and not-taken
branch conditions. Thus, all branches behave as they are always taken (including a full CPU pipeline flush). Enabling this feature
makes execution times more predictable while making timing side-channel attacks more difficult.
|             | When **disabled** not-taken conditional branches will execute faster without flushing the CPU pipeline. For highest
