...
| **2** (`ff`)         | <<_flipflop_based>>                        | provides a full hardware reset
| **3** (`latch`)      | <<_latch_based>>                           | improved efficiency for ASIC implementations
|=======================

.No Hardware Reset
[IMPORTANT]
Apart from the <<_flipflop_based>> style, the register file does not provide a dedicated hardware reset. Hence, all `x`
registers power-up in an undefined state and need software-based initialization (e.g. by the <<_start_up_code_crt0>>).
...
The register file can also be constructed from individual synchronous flip flops (edge triggered registers).
No flip flops are implemented for architecture register `x0`/`zero` which is hardwired to zero. In contrast to
the other architecture styles, this implementation flavor provides a **dedicated hardware reset** that
initializes all flip-flops to zero during system reset.
...
for **ASIC implementations** when based on standard cells.
...
or multiplications are processed by so-called "ALU co-processors".
...
The bus or load/store unit (VHDL file `neorv32_cpu_lsu.vhd`) takes care of handling data memory accesses via load and store
