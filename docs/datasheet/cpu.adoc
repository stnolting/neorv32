...
Access hosts like the CPU or the caches can request exclusive access to the downstream bus infrastructure using
the "bus lock" mechanism. While the bus is locked the locking host has exclusive access and can issue an arbitrary
number of transfers that cannot be interleaved by any other host.

The following figure shows a simplified read-followed-by-write transaction implemented as uninterruptible
exclusive bus access using the lock mechanism. Beyond others, the atomic read-modify-write operations use this concept.

[start=1]
. The lock request is sent by setting `lock` together with the first `stb` ("a"). `lock` stays high until the end of the entire read-write transfer.
. The bus is locked when the bus returns the first `ack` ("c").
...
.Exemplary "Atomic" Read-followed-by-Write Operation using the Bus Locking Mechanism
[wavedrom, format="svg", align="center"]
----
{signal: [
  {name: 'clk', wave: 'p......'},
  [
    "request",
    {name: 'addr',  wave: 'x3.4.x.'},
    {name: 'data',  wave: 'x..4.x.'},
    {name: 'ben',   wave: 'x..4.x.'},
    {name: 'stb',   wave: '01010..', node: '.a.d'},
    {name: 'rw',    wave: 'x0.1.x.'},
    {name: 'src',   wave: 'x0...x.'},
    {name: 'priv',  wave: 'x1...x.'},
    {name: 'debug', wave: 'x0...x.'},
    {name: 'amo',   wave: 'x0...x.'},
    {name: 'amoop', wave: 'x......'},
    {name: 'lock',  wave: 'x1...0x', node: '.b...f'},
    {name: 'fence', wave: '0......'},
  ],
  {},
  [
    "response",
    {name: 'ack',  wave: '0.1010.', node: '..c.eg'},
    {name: 'err',  wave: 'x.0x0x.'},
    {name: 'data', wave: 'x.3x...'},
  ]
],
 edge: ['ab', 'a~>c', 'c~>d', 'd~>e', 'e~>f', 'fg']
}
----
...
Read-modify-write operation from the <<_zaamo_isa_extension,`Zaamo`>> ISa extension will also force exclusive bus
access by setting the `lock` signal. The `amoop` signal specifies the actual atomic processing operation:
