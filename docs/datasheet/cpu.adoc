...
[WARNING]
The `Zicond` extension is not ratified yet. An intrinsic library is provided to utilize the `Zicond`
conditional operations from C-language code (see `sw/example/zicond_test`) even without compiler support.
...
`err` is high for exactly one cycle if the transaction failed to complete. These two signals are mutually exclusive.
