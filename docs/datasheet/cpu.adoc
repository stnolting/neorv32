...
The `Zbkx` sub-extension is part of the _RISC-V scalar cryptography_ ISA extension and adds crossbar permutation instructions.
...
The `Zknd` sub-extension is part of the _RISC-V scalar cryptography_ ISA extension and adds NIST AES decryption instructions.
...
The `Zkne` sub-extension is part of the _RISC-V scalar cryptography_ ISA extension and adds NIST AES encryption instructions.
...
The `Zknh` sub-extension is part of the _RISC-V scalar cryptography_ ISA extension and adds NIST hash function instructions.
...
The `Zknt` sub-extension is part of the _RISC-V scalar cryptography_ ISA extension and guarantees data independent execution
...
.2+<| `RVI` <| `lui` `auipc` `add[i]` `slt[i][u]` `xor[i]` `or[i]` `and[i]` `sub` <| always
            <| `sll[i]` `srl[i]` `sra[i]` <| only if `FAST_SHIFT_EN` enabled
| `RVM` | `mul[h]` `mulh[s]u` | always
.2+<| `RVC` <| `c.nop` `c.addi` `c.lui` `c.andi` `c.sub` `c.xor` `c.and` `c.mv` `c.add` <| always
            <| `c.srli` `c.srai` `c.slli` <| only if `FAST_SHIFT_EN` enabled
| `RVK` | `aes32ds[m]i` `aes32es[m]i` `sha256sig*` `sha512sig*` `sha512sum*` | always
.2+<| `RVB` <| `xperm4` `xperm8` `andn` `orn` `xnor` `pack[h]` `brev8` `rev8` <| always
            <| `ror[i]` `rol` <| only if `FAST_SHIFT_EN` enabled
|=======================
