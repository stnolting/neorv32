...
| 21    | _CSR_MSTATUS_TW_ | r/w | Timeout wait: raise illegal instruction exception when set if `WFI` instruction is executed outside of M-mode when set (only available if user mode is implemented)
