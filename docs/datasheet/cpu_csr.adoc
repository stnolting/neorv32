...
| 0x300 | <<_mstatush, `mstatus`>>  | `CSR_MSTATUS`    | MRW | Machine status register - low word
...
| 0x310 | <<_mstatush, `mstatush`>> | `CSR_MSTATUSH`   | MRW | Machine status register - high word
...
| Address     | `0x300` (`mstatus`)
|             | `0x310` (`mstatush`)
| Reset value | `0x00000000_00001800`
| ISA         | `Zicsr`
| Description | The `mstatus[h]` CSRs are used to configure general machine environment parameters.
|=======================
...
4+^| **statush`
| 10    | `CSR_MSTATUSH_MDT` | r/w | **MDT**: Machine-mode disable trap; see <<_double_trap_exception>>
|=======================
...
|=======================
| Bit   | Name [C] | R/W | Description
4+^| **<<_instruction_sets_and_extensions>>**
...
| 27    | -                     | r/- | _reserved_, hardwired to zero
4+^| **<<_cpu_tuning_options>>**
| 28    | `CSR_MXISA_RFHWRST`   | r/- | full hardware reset of register file available when set (`CPU_RF_HW_RST_EN`)
| 29    | `CSR_MXISA_FASTMUL`   | r/- | fast multiplication available when set (`CPU_FAST_MUL_EN`)
| 30    | `CSR_MXISA_FASTSHIFT` | r/- | fast shifts available when set (`CPU_FAST_SHIFT_EN`)
4+^| **Miscellaneous**
