...
regions, where each `pmpcfg*` CSR provides configuration bits for four regions. The actual number of available
`pmpcfg` CSRs and CSR entries is defined by the <<_pmp_num_regions>> generic. The following bits (for the
...
| 7   | `L` | r/w | lock bit, also enforces access rights in machine-mode, can only be cleared by CPU reset
| 6:5 |  -  | r/- | reserved, read as zero
| 4:3 | `A` | r/w | mode configuration; only **OFF** (`00`) and **TOR** (`01`) are supported
