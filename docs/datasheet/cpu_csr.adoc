...
|=======================
| Bit   | Name [C] | R/W | Description
| 27:0  | -                     | r/- | _reserved_, read as zero
| 28    | `CSR_MXCSR_RFHWRST`   | r/- | <<_cpu_tuning_options>>: full hardware reset of register file available when set (`CPU_RF_HW_RST_EN`)
| 29    | `CSR_MXCSR_FASTMUL`   | r/- | <<_cpu_tuning_options>>: fast multiplication available when set (`CPU_FAST_MUL_EN`)
| 30    | `CSR_MXCSR_FASTSHIFT` | r/- | <<_cpu_tuning_options>>: fast shifts available when set (`CPU_FAST_SHIFT_EN`)
...
| 31:28 | -                    | r/- | _reserved_, read as zero
|=======================
