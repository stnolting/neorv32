...
(for example: halting, resuming, single-stepping, memory access, executing arbitrary programs).However, work is still in progress.
...
| 28  | `ackhavereset` | -/w | write `1` to clear 'havereset' flag
...
* `001` - invalid read/write while command is/was executing
