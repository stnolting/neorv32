...
6+^| **Machine Physical Memory Protection**
...
| 0xb83 .. 0xb19f | `mhpmcounter3h` .. `mhpmcounter31h` | _CSR_MHPMCOUNTER3H_ .. _CSR_MHPMCOUNTER31H_ | r/w | Machine performance-monitoring counter 3..31 high word | 
...
6+^| **Machine Information Registers**
...
6+^| **NEORV32-Specific Custom Machine CSRs**
...
3+| Reset value: _0x00000020_
3+| The `mstatush` CSR is compatible to the RISC-V specifications. It provides additional CPU status
information. The following bits are implemented (all remaining bits are always zero and are read-only).
|======
