...
[TIP]
The documentation is automatically built and deployed to GitHub pages by a CI workflow (https://stnolting.github.io/neorv32/sw/files.html).
...
or to build a HTML-based documentation.

Pre-rendered pdf are available online as nightly pre-releases: https://github.com/stnolting/neorv32/releases.
The HTML-based documentation is also available online at the project' https://stnolting.github.io/neorv32/[GitHub Pages].

.Generate PDF documentation `docs/NEORV32.pdf` using `asciidoctor-pdf`
[source,bash]
----
neorv32$ make pdf
----

.Generate HTML documentation `docs/index.html` using `asciidoctor`
[source,bash]
----
neorv32$ make html
----
...
the clock requirements noted in <<_debug_transport_module_dtm>>.
...
at `690` in this example. Add this address as _breakpoint_ to GDB.

[NOTE]
The entry address might be different is you use a different version of the software framework or
if different CPU ISA features are configured.
