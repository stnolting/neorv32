...
.Supported AXI Transactions
[IMPORTANT]
The NEORV32 AXI4 bridge only supports single-accesses and incrementing address bursts.
Atomic / exclusive accesses are not supported and will raise a bus error.
...
(`neorv32_imem_image.vhd`) file in the packaged_ip folder. For the Vivado design suite, the new program to be executed must
...
.Incremental Compilation of Simulation Sources (XSIM)
[IMPORTANT]
